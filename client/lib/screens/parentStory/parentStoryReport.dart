...
class ParentStoryTimeReportScreen extends StatefulWidget {
  ...
  @override
  State<ParentStoryTimeReportScreen> createState() =>
      _ParentStoryTimeReportScreenState();
}
...
class _ParentStoryTimeReportScreenState extends State<ParentStoryTimeReportScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final report = await _api.getParentStoryTimeReport(
        token: token,
  ...
  double _maxMinutes() {
    ...
    for (final h in _histogram) {
      final raw = h["minutes"] ?? 0;
      final m = (raw is num) ? raw.toDouble() : double.tryParse("$raw") ?? 0.0;
      if (m > maxV) maxV = m;
    }
  ...
  Widget build(BuildContext context) {
                        ...
                        child: _stories.isEmpty
                            ...
                            : Column(
                                children: _stories.map((s) {
                                  ...
                                  final minsRaw = s["totalMinutes"] ?? 0;
                                  final mins = (minsRaw is num) ? minsRaw.toString() : (int.tryParse("$minsRaw") ?? 0).toString();
                                  ...
                                  if (latestReview != null && latestReview is Map) {
                                    ...
                                    final rInt = (rating is int) ? rating : int.tryParse("$rating");
                                    final stars = (rInt == null) ? "" : "⭐" * rInt;
                                    reviewText =
                                        "${supName ?? "Supervisor"}: $stars ${comment ?? ""}".trim();
    ...
    return Scaffold(
      ...
      body: kIsWeb ? _webWrapper(bodyContent) : bodyContent,
    );
  }
}
...
class _HistogramChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ على الويب: إذا الأيام كثير (30) خلي البارات أنحف
    final int n = histogram.length;
    final double rodWidth = (n >= 25) ? 8 : (n >= 14 ? 10 : 14);

    for (int i = 0; i < histogram.length; i++) {
      final raw = histogram[i]["minutes"] ?? 0;
      final m = (raw is num) ? raw.toDouble() : double.tryParse("$raw") ?? 0.0;
        ...
        BarChartData(
          ...
          titlesData: FlTitlesData(
            ...
            bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                showTitles: true,
                interval: (n >= 25) ? 3 : 1, // ✅ على 30 يوم: ما نكتب كل يوم
