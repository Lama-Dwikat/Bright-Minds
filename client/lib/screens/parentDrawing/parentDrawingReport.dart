...
class _ParentDrawingReportScreenState extends State<ParentDrawingReportScreen> {
  ...
  Widget _buildHistogram(List<dynamic> histogram) {
    ...
    final Map<String, int> secByDay = {for (final d in days) d: 0};

    for (final item in histogram) {
      final day = item["day"]?.toString();
      final sec = (item["totalSec"] ?? 0);
      final safeSec = (sec is int) ? sec : int.tryParse("$sec") ?? 0;

      if (day != null && secByDay.containsKey(day)) {
        secByDay[day] = safeSec;
      }
    }
      ...
      children: [
        ...
        Row(
          ...
          children: days.map((d) {
            ...
            final h = maxSec == 0 ? 0.0 : (sec / maxSec) * 120.0;
            final label = DateFormat("EEE").format(DateTime.parse(d));
            ...
            return Expanded(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(_fmtSec(sec), style: const TextStyle(fontSize: 11)),
                    const SizedBox(height: 6),
                    Container(
                      height: 120,
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => _ParentDrawingFullScreen(
          ...
          imageBase64: (drawing["imageBase64"] ?? "").toString(),
          createdAt: drawing["createdAt"]?.toString(),
          durationSec: (drawing["durationSec"] ?? 0) is int
              ? (drawing["durationSec"] ?? 0) as int
              : int.tryParse("${drawing["durationSec"]}") ?? 0,
        ),
      ),
    );
  }
  ...
  Widget _webWrapper(Widget child) {
    // مرتب على المتصفح (بدون ما يمدد كرتات كثير)
    return Center(
  ...
  Widget build(BuildContext context) {
        ...
        : report.isEmpty
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  final kid = report[index] as Map<String, dynamic>;

                  final childName = (kid["childName"] ?? "Child").toString();
                  final ageGroup = (kid["ageGroup"] ?? "").toString();

                  final totalSecRaw = kid["totalSec"] ?? 0;
                  final activitySecRaw = kid["activitySec"] ?? 0;

                  final totalSec = (totalSecRaw is int) ? totalSecRaw : int.tryParse("$totalSecRaw") ?? 0;
                  final activitySec = (activitySecRaw is int) ? activitySecRaw : int.tryParse("$activitySecRaw") ?? 0;
                  ...
                  return Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            "$childName${ageGroup.isNotEmpty ? " ($ageGroup)" : ""}",
                          ...
                          if (drawings.isEmpty)
                            ...
                            ...drawings.take(10).map((d) {
                              ...
                              final created = dm["createdAt"] != null
                                  ? DateTime.tryParse(dm["createdAt"].toString())
                                  : null;
                              ...
                              final secRaw = dm["durationSec"] ?? 0;
                              final sec = (secRaw is int) ? secRaw : int.tryParse("$secRaw") ?? 0;

                              final title = (dm["activityTitle"] ?? "Drawing").toString();

                              return ListTile(
                                contentPadding: EdgeInsets.zero,
                                title: Text(title),
                                subtitle: Text("$createdText  •  ${_fmtSec(sec)}"),
                                trailing: const Icon(Icons.chevron_right),
                                onTap: () => _openDrawingFullScreen(dm),
                              );
                            }).toList(),
                        ],
                      ),
    ...
    return Scaffold(
      ...
      body: kIsWeb ? _webWrapper(content) : content,
    );
  }
