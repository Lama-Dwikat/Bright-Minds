...
// ✅ mobile-only (safe because guarded with kIsWeb)
...
class _SupervisorSettingsScreenState extends State<SupervisorSettingsScreen> {
  ...
  // ✅ CV Viewer: Web opens new tab, Mobile downloads temp and opens
  Future<void> _viewCv() async {
    if (_token == null) return;

    final url = Uri.parse("${getBackendUrl()}/api/users/my-cv");

    try {
      if (kIsWeb) {
        // ✅ open in new tab with token as header is not possible
        // so use a token query param (best) OR create public signed url endpoint
        // Here: we try opening a web route that supports token as query
        // If your backend doesn't support it, I'll tell you below.
        final webUrl = Uri.parse("${getBackendUrl()}/api/users/my-cv?token=$_token");
        ...
        if (!ok) _snack("Could not open CV on web");
        return;
      }

      // ✅ Mobile: fetch bytes with Authorization header, save, open
      final resp = await http.get(
        url,
  ...
  // ✅ Web wrapper: center and fixed max width
  Widget _webWrapper({required Widget child}) {
    return Center(
  ...
  Widget build(BuildContext context) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (kIsWeb) ...[
                        const SizedBox(height: 8),
                        const Text(
                          "Note: On Web, CV opens in a new tab.",
                          style: TextStyle(color: Colors.black54),
                        ),
                      ],
                    ],
    ...
    return Scaffold(
      ...
      body: kIsWeb ? _webWrapper(child: content) : content,
    );
  }
}
