...
class _ParentSettingsScreenState extends State<ParentSettingsScreen> {
  ...
  Future<void> _fetchKids() async {
    ...
    if (mounted) setState(() => _loadingKids = true);
    try {
      final resp = await http.get(
  ...
  Future<void> _editParentField(String field, String label) async {
    ...
    final controller =
        TextEditingController(text: (_parent?[field] ?? "").toString());
  ...
  Widget _avatar() {
    final name = (_parent?["name"] ?? "P").toString();
    return CircleAvatar(
      radius: 52,
  ...
  // ✅ Web wrapper: يجعل الصفحة مرتبة وما تتمدد كثير
