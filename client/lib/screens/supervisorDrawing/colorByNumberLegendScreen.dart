...
class _ColorByNumberLegendScreenState extends State<ColorByNumberLegendScreen> {
  ...
  Future<void> _load() async {
    ...
    // ✅ مهم: على الويب خليها من getBackendUrl (مش 10.0.2.2)
    _baseUrl = prefs.getString("backendUrl") ?? getBackendUrl();
  ...
  Widget _buildHeaderCard() {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            const Icon(Icons.touch_app, color: AppColors.softSunYellow),
            const SizedBox(width: 10),
            Expanded(
              child: Text(
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: AppColors.softSunYellow.withOpacity(0.14),
                borderRadius: BorderRadius.circular(14),
                border: Border.all(color: AppColors.softSunYellow.withOpacity(0.28)),
              ),
              child: Text(
  ...
  Widget _buildLegendGrid({required int crossAxisCount}) {
              ...
              children: [
                Container(
                  ...
                  child: Text(
                    ...
                    style: GoogleFonts.robotoSlab(
                      fontWeight: FontWeight.w900,
                      color: (color.computeLuminance() < 0.45) ? Colors.white : Colors.black,
                      fontSize: 20,
                    ),
                  ),
                ),
  ...
  // ✅ WEB layout: small image left, colors right
  Widget _buildWebLayout(String? imageUrl) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final w = constraints.maxWidth;

        // صورة صغيرة: ~30% من العرض (مع حد أقصى)
        final imgWidth = (w * 0.32).clamp(240.0, 360.0);

        // كل ما الشاشة تكبر زيد الأعمدة
        final int cols = w >= 1200 ? 4 : 3;
          ...
          child: Row(
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  children: [
                    // على الويب خلّي الشبكة تتمدّد داخل Expanded
                    Expanded(
                      child: _buildLegendGrid(crossAxisCount: cols),
                    ),
                  ],
                ),
              ),
  ...
  // ✅ MOBILE layout: keep as-is (image top + header + grid)
  Widget _buildMobileLayout(String? imageUrl) {
    return Column(
      children: [
        ...
        _buildHeaderCard(),
        Expanded(child: _buildLegendGrid(crossAxisCount: 3)),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: kIsWeb ? _buildWebLayout(imageUrl) : _buildMobileLayout(imageUrl),
      ),
    );
  }
}
