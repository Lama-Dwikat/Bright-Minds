...
class _ColorByNumberPlayScreenState extends State<ColorByNumberPlayScreen> {
  ...
  Future<void> _load() async {
    ...
    // ✅ بس للويب: لو ما عندك backendUrl محفوظ، localhost هو الافتراضي
    _baseUrl = prefs.getString("backendUrl") ??
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ WEB layout فقط (ولا يلمس الموبايل)
    return LayoutBuilder(
      builder: (context, c) {
        final isWebWide = kIsWeb && c.maxWidth >= 1000;

        if (!isWebWide) {
          // ================== MOBILE UI (كما هو تماماً) ==================
          return Scaffold(
            appBar: AppBar(
                  ...
                  child: Center(
                    child: ConstrainedBox(
                      ...
                      child: Container(
                        ...
                        child: _buildCanvas(), // نفس الدالة، بس داخل Card
                      ),
                    ),
                  ),
                ),
  ...
  Widget _legendGrid() {
    return GridView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        final textColor =
            (color.computeLuminance() < 0.45) ? Colors.white : Colors.black;
...
class _CanvasPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final s in strokes) {
      ...
      canvas.drawCircle(s.p, s.radius, paint);
    }
