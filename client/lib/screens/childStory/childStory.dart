...
class _StoryKidsState extends State<StoryKidsScreen> {
  ...
  // ================= RESPONSIVE HELPERS (WEB + MOBILE) =================
  int _gridCountForWidth(double w) {
    if (w < 600) return 2; // mobile
    if (w < 900) return 3; // small web/tablet
    if (w < 1200) return 4; // medium web
    return 5; // large web
  }

  double _maxContentWidth(double w) {
    if (w >= 900) return 1100; // center content on web
    return w;
  }

  double _childAspectRatioForWidth(double w) {
    if (w >= 900) return 0.80; // nicer cards on web
    return 0.75; // keep your mobile ratio
  }
  ...
  void _showReviewDialog(Map<String, dynamic> review) {
    ...
    String supervisorName =
        review["supervisorId"]?["name"] ?? "Unknown Supervisor";
  ...
  // ✅ changed: now takes width for responsive grid
  Widget _buildStoriesList(double width) {
    if (_isLoading) {
    ...
    return GridView.builder(
      ...
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
        ...
        crossAxisSpacing: 16,
        mainAxisSpacing: 16,
        childAspectRatio: _childAspectRatioForWidth(width),
      ),
              ...
              onTap: () async {
                ...
                if (status == "approved" || status == "pending") {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) =>
                          ReadOnlyStoryPage(storyId: story['_id']),
                    ),
                  );
                } else {
                          ...
                          Row(
                            children: [
                              ...
                              if (story['reviews'] != null &&
                                  story['reviews'].isNotEmpty)
                                GestureDetector(
                                  onTap: () =>
                                      _showReviewDialog(story['reviews'][0]),
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        ...
        Positioned(
          bottom: 100,
          right: kIsWeb ? 30 : 10, // optional: nicer on web
          child: GestureDetector(
            onTap: () async {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => CreateStoryPage(storyId: null)),
              ).then((_) => _fetchChildStories());
            },
            child: Container(
              width: 75,
              height: 75,
              decoration: BoxDecoration(
                color: const Color.fromARGB(255, 250, 206, 148),
