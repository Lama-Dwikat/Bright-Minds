...
class _ParentKidWeeklyChallengesScreenState
  ...
  Future<void> _fetchWeek() async {
    ...
    try {
      ...
      final url = Uri.parse(
        "${getBackendUrl()}/api/challenges/parent/kid-week"
        "?kidId=${widget.kidId}&weekStart=$weekStartStr",
      );
      ...
      if (resp.statusCode == 200) {
        ...
        setState(() {
          _kid = data["kid"] is Map
              ? Map<String, dynamic>.from(data["kid"])
              : null;
          _plan = (data["plan"] is Map) ? Map<String, dynamic>.from(data["plan"]) : null;
  ...
  bool _isDone(int dayIndex) {
    for (final x in _progress) {
      if (x["dayIndex"] == dayIndex) {
        return x["done"] == true;
      }
    }
    return false;
  ...
  // ✅ safer than firstWhere(orElse: null)
  Map<String, dynamic>? _templateForDay(int dayIndex) {
    ...
    Map? match;
    for (final d in days) {
      if (d is Map && (d["dayIndex"] ?? -1) == dayIndex) {
        match = d;
        break;
      }
    }
                ...
                Row(
                  children: [
                    Icon(
                      ...
                      color:
                          done ? AppColors.warmHoneyYellow : Colors.black38,
                    ),
