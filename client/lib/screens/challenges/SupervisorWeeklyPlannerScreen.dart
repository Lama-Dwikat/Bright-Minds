...
class _SupervisorWeeklyPlannerScreenState
  ...
  Future<void> _fetchKidsForSupervisor() async {
    ...
    try {
      final resp = await http.get(
        Uri.parse(
            "${getBackendUrl()}/api/users/kidsForSupervisor/$_supervisorId"),
        headers: {
          "Content-Type": "application/json",
      ...
      if (mounted) setState(() => _kids = []);
    } finally {
      if (mounted) setState(() => _loadingKids = false);
    }
  }
  ...
  Future<void> _createTemplateDialog() async {
    ...
    String selectedCategory =
        _categories.firstWhere((c) => c != "All", orElse: () => "reading");
  ...
  Future<void> _randomWeek() async {
    ...
    try {
      ...
      if (mounted) setState(() => _randoming = false);
    }
  }
  ...
  Color _badgeColor(String category) {
    switch (category) {
      case "religious":
        return AppColors.warmHoneyYellow.withOpacity(0.18);
      case "reading":
        return AppColors.pastelYellow.withOpacity(0.22);
      case "health":
        return AppColors.softSunYellow.withOpacity(0.22);
      case "sport":
        return AppColors.goldenYellow.withOpacity(0.16);
      case "behavior":
        return AppColors.quoteCard.withOpacity(0.20);
      case "art":
        return AppColors.quoteInner.withOpacity(0.55);
      case "nature":
        return AppColors.warmHoneyYellow.withOpacity(0.14);
      default:
  ...
  Widget _dayCard({
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: highlight ? AppColors.creamYellow : Colors.white,
        borderRadius: BorderRadius.circular(18),
        border: Border.all(
          color: highlight ? AppColors.warmHoneyYellow : Colors.black12,
          width: highlight ? 2 : 1,
        ),
        boxShadow: [
          BoxShadow(
  ...
  double _maxWidthWeb(double w) {
    if (!kIsWeb) return w;
    if (w >= 1400) return 860;
    if (w >= 1200) return 820;
    if (w >= 900) return 760;
    return w;
  }

  // ✅ زر موحد شكله حلو وما ينضغط
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                // ✅ Responsive buttons (Wrap) - ما بنضغطوا
                                Wrap(
                                  spacing: 10,
                                  runSpacing: 10,
                                  children: [
                          ...
                          // WEEK BOARD (horizontal scroll as before)
                          SizedBox(
                            height: 220,
                          ...
                          // ✅ Templates list inside the same scroll
...
class _SupervisorWeeklyPlannerScreenState
  ...
  Future<void> _createTemplateDialog() async {
    final titleCtrl = TextEditingController();
    String selectedCategory = _categories.firstWhere((c) => c != "All",
        orElse: () => "reading");
  ...
  Color _badgeColor(String category) {
    // ✅ make all badges match Yellow Theme (no green/blue/purple)
    switch (category) {
      case "religious":
        return AppColors.warmHoneyYellow.withOpacity(0.18);
      case "reading":
